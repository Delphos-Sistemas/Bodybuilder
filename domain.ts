...
export default function HistoricoPage() {
  ...
  return (
    <div className="space-y-5">
      <MascotMessage variant="achievement" title="Histórico" message="Não persiga apenas números. Construa capacidade." />
          ...
          return (
            <Card key={session.id} className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-sm text-sand">{session.sets.length} séries · volume {session.totalVolume.toFixed(0)} kg · dificuldade {session.difficulty ?? "-"}/10</p>
              </div>
